"""
Maps each perceptual category to the frequency ranges to attenuate when
that category is muted, plus how much (in dB). These are starting guesses
based on typical spectral content -- expect to retune by ear (Step 3).
...
FREQUENCY_BANDS = {
    ...
    # also guts any concurrent speech -- since keeping speech clear is the
    # project's actual priority, these are deliberately mild everywhere
    # except the sub-250Hz rumble (which speech barely touches): traffic
    # gets noticeably reduced, not eliminated, when it overlaps speech.
    "Traffic": [
    ...
    "Speech": [],    # not suppressed on its own in this phase
    "Chatter": [],   # intentionally empty -- Chatter is spectrally identical to Speech,
                     # so it's suppressed by amplitude (see suppress.chatter_duck_gain),
                     # not by cutting frequencies that direct speech also relies on
    "Other": [],     # never suppressed
}
