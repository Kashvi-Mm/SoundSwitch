"""
SoundSwitch - Real-time mic demo (frequency-band suppression only, no
speech/chatter classification -- see soundswitch/streaming.py).
...
CATEGORIES = ["Traffic", "Mechanical Hums"]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Uniform gain multiplier applied to incoming audio before processing (e.g. 3.0 "
             "= 3x louder). Makes quiet mic pickup audible enough to judge by ear.",
    )
...
if __name__ == "__main__":
    ...
    suppressor = StreamingSuppressor(args.mute, sr=samplerate, input_gain=args.input_gain)
